...
async def main():
    ...
    parser.add_argument('-v', '--exclude', nargs='+', help='除外するURLの文字列（スペース区切りで複数指定可能）')
    args = parser.parse_args()

    mirror = AsyncWebMirror(args.url, args.output_dir, max_connections=args.connections, weights=args.weights, exclude=args.exclude)
