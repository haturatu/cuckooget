...
table1: vec![None; size],
  ...
  pub fn insert(&mut self, key: String, value: String) -> bool {
    ...
    for _ in 0..self.max_kicks {
      ...
      if rand::random::<bool>() {
        std::mem::swap(&mut self.table1[h1], &mut Some((key.clone(), value.clone()))); // keyとvalueをクローン
      } else {
        std::mem::swap(&mut self.table2[h2], &mut Some((key.clone(), value.clone()))); // keyとvalueをクローン
      }
    }
